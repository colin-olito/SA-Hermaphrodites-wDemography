...
calcZeta  <-  function(om, FSi, FXi, FXi_pr, USi, UXi, pHat_AA, pHat_aa, C, delta){
	...
	pn_aa   <-  ones(c(om)) %*% FXi_pr[,,3] %*% (pHat_aa[(4*om+1):(5*om)] + pHat_aa[(5*om+1):(6*om)])
	lambda_i  <-  c(0,0)
	lambda_i[1]  <- ones(2*om) %*% rbind(
					cbind(USi[,,1] + FSi[,,1], FSi[,,1]),
					cbind(FXi[,,1], UXi[,,1] + FXi[,,1])
					) %*% pHat_AA[1:(2*om)]
	lambda_i[2]  <- ones(2*om) %*% rbind(
					cbind(USi[,,3] + FSi[,,3], FSi[,,3]),
					cbind(FXi[,,3], UXi[,,3] + FXi[,,3])
					) %*% pHat_aa[(4*om+1):(6*om)]#-
	M22_AA  <-  rbind(
	...
	rhoM22    <-  c(max(Re(eigen(M22_AA, symmetric=FALSE, only.values = TRUE)$values)),
					 max(Re(eigen(M22_aa, symmetric=FALSE, only.values = TRUE)$values)))
	zeta_i  <-  rhoM22/lambda_i
	results  <-  list("lambda_i" = lambda_i,
					  "rhoM22"   = rhoM22,
					  "zeta_i"   = zeta_i)
	return(results)
}
...
#' delta:	Early-acting inbreeding depression (proportion ovules aborted due to I.D.)
#' delta_j:	Proportional decrease in juvenile survival rate for inbred offspring
#' delta_a:	Proportional decrease in adult survival rate for inbred offspring
#' delta_gamma: Proportional decrease in juv. --> adult transition rate for inbred offspring
#' tlimit:	Max # of generations for fwd simulation
#' Ainvade:	Should A allele start off rare
#' intInit:	Optional initial frequency of A allele
...
#		points(AInvBound[i] ~ sms[i], cex=1)
	...
	# File name component for presence/absence of I.D.
	IDpres  <-  "T"
	if(all(delta.list == 0)) {
		IDpres  <-  "F"
	}
...
##############################
#' Titrate to find extinction threshold across sf x sm parameter space
#'
