...
\begin{quote}
	{\itshape We agree that less focus on the (derivation of) general model and more focus on the specific two stage example will hopefully make the paper more accessible for non-theoretical readers, and we have  restructured the methods accordingly. }
\end{quote}
...
\begin{quote}
	{\itshape We agree that in our efforts to appeal to both demographers and evolutionary biologists studying sexual conflict, we ended up insufficiently discussing the literature on both accounts. We have attempted to improve this, see also our response to reviewer 1. }
\end{quote}
...
\begin{quote}
	{\itshape We want to thank the reviewer for this suggestion. We now start with a figure that only has one f, and that separately explains what happens to the allele frequencies (Population genetic outcome) and to the population growth rate (Demographic Outcome), see new Figure 1. We hope that this will make it easier to interpret the old figure 1, which is now Figure 2. We have also included a new supplementary figure (CHECK FIG NUMBERING IN SUPPLEMENTS) with detailed heatmaps for each of the parameter combinations presented in the this figure (i.e., one f per panel). However, we felt that this revised figure was to large and complicated to present in the main text (18 vs. 6 panels).}
\end{quote}
